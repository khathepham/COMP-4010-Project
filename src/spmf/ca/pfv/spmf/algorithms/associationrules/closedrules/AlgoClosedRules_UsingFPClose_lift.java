 ...
 * Here it is adapted for mining closed rules using the results obtained from
 * the FPClose algorithm (the frequent
 * closed itemsets).
 * The main difference with AlgoAgrawalFaster94.java, is that this
 * implementation takes
 * a different type of Itemset object as input. Moreover, the method
 * CALCULATESUPPORT() is
 * different because if an itemset is not closed, we must find its closure to
 * determine
 * its support.
 * To implement the calculation of the support efficiently, this class uses the
 * CFI-tree
...
public class AlgoClosedRules_UsingFPClose_lift extends AlgoAgrawalFaster94 {
	 ...
	 * Default constructor
	 */
	public AlgoClosedRules_UsingFPClose_lift() {
	 ...
	 * An alternative would be to use a trie to store patterns but it may require a
	 * bit more memory.
	 * 
