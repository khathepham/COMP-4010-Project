 ...
 * using
 * fp-trees. Knowledge and Data Engineering, IEEE Transactions on, 17(10),
 * 1347-1362.
 * * <br/>
 * <br/>
 * 
 ...
 */
public class AlgoFPClose_inverse {
	...
	public int maxSupportRelative;// the relative minimum support
	 ...
	 */
	public Itemsets runAlgorithm(String input, String output, double minsupp, double maxsupp)
		...
		FPTree_inverse tree = new FPTree_inverse();
		...
		while (((line = reader.readLine()) != null)) {
			...
			for (String itemString : lineSplited) {
				...
				if (originalMapSupport.get(item) >= minSupportRelative
						&& originalMapSupport.get(item) <= maxSupportRelative) {
	 ...
	 */
	private void fpclose(FPTree_inverse tree, int[] prefix, int prefixLength, int prefixSupport,
		...
		if (singlePath && countBuffer[position - 1] >= minSupportRelative
				&& countBuffer[position - 1] <= maxSupportRelative) {
			// System.out.println();
			...
			for (int i = tree.headerList.size() - 1; i >= 0; i--) {
				...
				if (cfiTree.passSubsetChecking(headWithP, prefixLength + 1, betaSupport)) {
					...
					// Create the tree.
					FPTree_inverse treeBeta = new FPTree_inverse();
					// Add each prefixpath in the FP-tree.
					for (List<FPNode> prefixPath : prefixPaths) {
						treeBeta.addPrefixPath(prefixPath, mapSupportBeta, minSupportRelative, maxSupportRelative);
					}
